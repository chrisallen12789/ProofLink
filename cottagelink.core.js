(() => {
  const config = window.COTTAGELINK_CONFIG || {};
  ...
  function loadCatalogCache(storage = window.sessionStorage) {
    const parsed = safeParse(storage.getItem(cartConfig.catalogCacheKey || "cottagelink_public_catalog_v1") || "[]");
  ...
  function saveCatalogCache(rows, storage = window.sessionStorage) {
    try {
      storage.setItem(cartConfig.catalogCacheKey || "cottagelink_public_catalog_v1", JSON.stringify(rows || []));
    } catch {
