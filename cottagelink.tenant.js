...
(function () {
  ...
  function waitForTenantReady(timeoutMs = 4000) {
    if (window.COTTAGELINK_TENANT_READY && typeof window.COTTAGELINK_TENANT_READY.then === "function") {
      return window.COTTAGELINK_TENANT_READY;
    }
    ...
    return new Promise((resolve) => {
      ...
      const finish = () => {
        ...
        resolve(window.COTTAGELINK_TENANT);
      };
      const timeoutId = window.setTimeout(finish, timeoutMs);
      document.addEventListener("prooflink:tenant-ready", finish, { once: true });
      window.setTimeout(() => {
        if (window.COTTAGELINK_TENANT_READY && typeof window.COTTAGELINK_TENANT_READY.then === "function") {
          Promise.resolve(window.COTTAGELINK_TENANT_READY).then(finish).catch(finish);
        }
      }, 0);
    });
  }
  ...
  async function hydrateTenant() {
    ...
    if (!selector && /^(prooflink\.co|www\.prooflink\.co|127\.0\.0\.1|localhost)$/i.test(window.location.hostname || "")) {
      return window.COTTAGELINK_TENANT;
    }

    try {
      const res = await fetch(`/.netlify/functions/get-public-tenant-info${selector ? `?${selector}` : ""}`);
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data?.tenant) return window.COTTAGELINK_TENANT;
      deepMerge(window.COTTAGELINK_TENANT, data.tenant);
      if (typeof window.COTTAGELINK_REFRESH_CONFIG === "function") {
        window.COTTAGELINK_REFRESH_CONFIG();
      }
      document.dispatchEvent(new CustomEvent("prooflink:tenant-ready", { detail: window.COTTAGELINK_TENANT }));
      return window.COTTAGELINK_TENANT;
    } catch (error) {
  ...
  window.COTTAGELINK_TENANT_READY = hydrateTenant();
