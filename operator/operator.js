...
// Extracted into operator-assistant-workspace.js and lazy-loaded on demand.
async function fetchAndRenderMessages() {
  await ensureOperatorWorkspaceScript?.("messages");
  return window.PROOFLINK_OPERATOR_ASSISTANT_WORKSPACE?.fetchAndRenderMessages?.();
}
...
async function initAIPanel() {
  await ensureOperatorWorkspaceScript?.("ai");
  return window.PROOFLINK_OPERATOR_ASSISTANT_WORKSPACE?.initAIPanel?.();
}
